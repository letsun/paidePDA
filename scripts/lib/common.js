...
(function($) {
	...
	common.alert = function(cfg) {
		...
		con +=
			'-webkit-transition: ease-out 0.5s; -moz-transition: ease-out 0.5s;-ms-transition: ease-out 0.3s; -o-transition: ease-out 0.3s;' +
			'transition: ease-out 0.3s;z-index:9999;opacity:0"><div style="position: absolute;top: 40%;left:50%;width: ' +
			width +
		...
		if (cfg.title) {
			con += '<div style="font-size: 23px;line-height: 60px;text-align: center;color: #60a0ff;">' + cfg.title + '</div>' +
				'<div style="font-size: 23px;color: #555;padding: 10px;text-align:' + textAlign +
				';border-bottom: 1px solid #ccc;' +
				'word-break:break-all;word-wrap:break-word;position:relative">' + cfg.content + '</div>';
		} else {
			con += '<div style="font-size: 23px;color: #555;padding: 30px 10px;text-align:' + textAlign +
				';border-bottom: 1px solid #ccc;' +
				'word-break:break-all;word-wrap:break-word;position:relative">' + cfg.content + '</div>';
		}
		...
		if (dialog) {
			con +=
				'<div><button style="width: 48%;height: 60px;border: none;background: none;font-size: 23px;padding: 0;outline: none" ' +
				'id="dCancel' + id + '">' + cancelValue +
				'</button><button style="width: 48%;height: 60px;border: none;background: none;' +
				'font-size: 26px;padding: 0;color: #60a0ff;outline: none;" id="dConfirm' + id + '">' + okValue +
				'</button></div></div>';
		} else {
